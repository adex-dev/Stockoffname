...
export function style() {
    ...
    $.ajax({
      ...
      error: function (jqXHR, error, errorThrown) {
        if (jqXHR.status && jqXHR.status == 500) {
          let text = "Periksa Kembali isi file anda,pastikan tidak ada spesial character";
  ...
  $.ajax({
    ...
    data: { tgl: localStorage.getItem('tanggal'), store: localStorage.getItem('store') },
    dataType: "json",
    success: function (response) {
    ...
    $.ajax({
      ...
      error: function (jqXHR, error, errorThrown) {
        if (jqXHR.status && jqXHR.status == 500) {
          ...
          let isi = "Berhasil Ditambahkan";
          sukses(isi).then((response) => {
            Swal.close();
          });
        }
      },
