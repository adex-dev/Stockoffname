        ...
        if ($sukses) {
          ...
          if ($connected) {
            ...
            foreach ($apikeystore as $value) {
              if ($this->db->get_where('prm_auditor',['kode_store' => $value->kode_store,'status_audit'=>'SELESAI'])->num_rows() > 0) {
              } else {
  ...
  public function postserveronline()
  {
    ...
    if ($this->input->post()) {
      ...
      if ($rs->num_rows() > 0) {
        ...
        if ($cek['respon'] == '404') {
          $msg = ['respon' => '404', 'messages' => $cek['messages']];
        } else if ($cek['respon'] == '200') {
          $getstok = $this->db->get_where('prm_stock', ['periode' => $tanggal, 'kode_store' => $store])->result();
          foreach ($getstok as $value) {
            $sukses = $this->aut->posstock($store, $tanggal, $value->ean, $value->nama_store, $value->item_id, $value->waist, $value->inseam, $value->item_description, $value->category, $value->kelas, $value->subkelas, $value->bin, $value->onhand_qty, $value->onhand_scan, $value->status);
          }
          if ($sukses) {
            $msg = ['respon' => '200', 'messages' => 'Sukses Ditambahkan'];
          }
        }
      } else {
